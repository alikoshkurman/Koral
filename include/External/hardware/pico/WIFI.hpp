...
namespace Koral::hardware::pico::WIFI {
  ...
  struct Init {
    ...
    ~Init() noexecpt {
      if(cyw43_aech_deinit()) {
        Koral::out.error("Koral::firmware::pico::WIFI: Can't deinitialise Wi-Fi.\n");
  ...
  struct Pin {
    ...
    bool toggle() noexcept {
      state = !state;
      if(cyw43_arch_gpio_put(pin, state)) {
        Koral::out.error("Koral::firmware::pico::WIFI: Can't initialise Wi-Fi pin.\n");
        return false;
      } 
      return true;
    }
  ...
  namespace Pin {
    inline uint32_t LED() noexcept {
      return 0;
    }
    inline uint32_t SMPS() noexcept {
      return 1;
    } 
    inline uint32_t VBUS() noexcept {
      return 2;
    } 
  }
